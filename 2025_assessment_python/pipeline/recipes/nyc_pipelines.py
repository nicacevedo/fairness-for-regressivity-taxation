...
class NYCMassAppraisalPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def sales_filter_mask(
        ...
        """
        Suggested TRAINING filter mask.
        """
        d = self._ensure_df(df)
        d = self._normalize_columns(d)
        y = self._parse_currency_to_float(d.get(self.target_col, np.nan))
    ...
    def _prepare_features(self, df: pd.DataFrame, is_fit: bool) -> pd.DataFrame:
        ...
        # --- FIX: Parse dates FIRST so they are available for engineering ---
        if self.sale_date_col in d.columns:
            d[self.sale_date_col] = self._parse_date(d[self.sale_date_col])

        if self.add_time_features and self.sale_date_col in d.columns:
            dt = d[self.sale_date_col]
            d["sale_year"] = dt.dt.year
            d["sale_month"] = dt.dt.month
            d["sale_quarter"] = dt.dt.quarter
            # cyc month
        ...
        # 3) Parse numeric-like object columns
        for c in [
            "sale_price", "land_square_feet", "gross_square_feet",
            "lotarea", "bldgarea", "comarea", "resarea", "officearea", "retailarea",
            "garagearea", "strgearea", "factryarea", "otherarea",
    ...
    def _feature_engineering(self, d: pd.DataFrame) -> pd.DataFrame:
        ...
        # Now sale_year should exist because we moved date parsing up
        sale_year = num("sale_year")
        df["age_at_sale"] = sale_year - year_built
        df["age_at_sale"] = df["age_at_sale"].clip(lower=0)
        ...
        # Clip max ratio to 10.0 to prevent infinity/explosion
        df["builtfar_to_residfar"] = (builtfar / (residfar + eps)).clip(upper=10.0)
        df["builtfar_to_commfar"] = (builtfar / (commfar + eps)).clip(upper=10.0)
        df["is_commercial_zone"] = (commfar > 0).astype(int)
        # --------------------------------
    ...
    def _default_drop_cols(self, df: pd.DataFrame) -> set:
        drop = {
            ...
            # --- FIX: Expanded drop list for redundancy & leakage ---
            # Redundant Geographies
            "bctcb2020", "bct2020", "tract2010", "census_tract_2010", 
            "cb2010", "community_board_dup1", "council_district_dup1", 
            "borough_dup1", "latitude_dup1", "longitude_dup1",
            
            # Redundant Physicals (We engineered clean versions)
            "land_square_feet", "gross_square_feet", 
            "bldgclass", # Redundant with building_class_at_time_of_sale
            
            # Leakage / Tax Administrative (Assessments are often based on sales)
            "assesstot", "assessland", "exempttot", 
            "tax_class_as_of_final_roll", # Use 'at_time_of_sale' version
            # --------------------------------------------------------
        }
