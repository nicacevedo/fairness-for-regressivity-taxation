...
class GroupDeviationConstrainedLogisticRegression:
    ...
    def fit(self, X, y, y_real_values=None):
        ...
        if self.objective == "mse" and self.constraint == "max_mse":
            ...
            # Upper bound (depending on the model)
            ...
            t0 = time()
            try:
                delta_J_lb_exp = primal_prob.solve(solver=self.solver, verbose=False)
            except cp.error.SolverError:
                print(f"{self.solver} not available, trying default solver.")
                delta_J_lb_exp = primal_prob.solve(verbose=True)
            print(fr"POF J % LB (exp): ", delta_J_lb_exp / J_0)
            ...
            t0 = time()
            try:
                delta_J_ub_exp = primal_prob.solve(solver=self.solver, verbose=False)
            except cp.error.SolverError:
                print(f"{self.solver} not available, trying default solver.")
                delta_J_ub_exp = primal_prob.solve(verbose=True)
            print(fr"POF J % UB (exp): ", delta_J_ub_exp / J_0)
            print(f"Root find for t_LB={t_LB.value} in: ", time() - t0)
        ...
        return result, solve_time, price_of_fairness, fairness_effective_improvement, delta_J_taylor_lb/ J_0, delta_J_taylor_ub/ J_0, delta_J_taylor/ J_0, real_tau
